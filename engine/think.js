...
// Generators
clickGenerator.addEventListener("click", () => generatorOnAction(GENERATOR_IDS.CLICK));
...
function consumePoints(pointsToConsume) {
    ...
    for (const [key, valueToConsume] of Object.entries(pointsToConsume)) {
        ...
        for (let i = proxySave.points_order.length - 1; i >= 0 && pointsOrderItemsToRemove > 0; i--) {
...
function checkUnlocks() {
    GENERATORS.forEach(generator => {
        ...
        let currentGenerator = proxySave.generators.find(value => value.name === generator.name);
        ...
        if (currentGenerator.built) {
            ...
            utils.addEventListenerWithFlag(generatorElement, 'click', generatorOnAction, currentGenerator.name);
            
            return;
        }

        if (currentGenerator.progress ||
            (generator['unlockRequires']['build'] &&
            hasEnoughPoints(generator['unlockRequires']['build']))) {

            if (!currentGenerator.canBuild)
                currentGenerator.canBuild = true;

            const generatorElement = document.getElementById(generator.name) ?? render.renderGenerator(generator.name);
        ...
        if (generator['unlockRequires']['hint'] &&
            hasEnoughPoints(generator['unlockRequires']['hint'])) {

            if (!currentGenerator.hinted)
                currentGenerator.hinted = true;
...
function buildGenerator(generatorName) {
    ...
    const currentGenerator = proxySave.generators.find(value => value.name === generator.name);

    if (!currentGenerator || currentGenerator.built || !currentGenerator.canBuild) return;
...
function generatorOnAction(generatorName) {
    const generator = proxySave.generators.find(generator => generator.name == generatorName);
    if (!generator) return;
...
async function setStoragePoints(points, orderedPoints) {
    ...
    orderedPoints.forEach((pointType, index) => {
        const child = childrenArray[index];
        
        // Determine the correct class for the point type
        let expectedClass = POINT_CLASSES[pointType];

        // Check if the current child has the correct class
        if (!child.classList.contains(expectedClass)) {
            // Replace the current child with the correct one
            const newChild = document.createElement('div');
            newChild.classList.add(expectedClass);
        ...
        if (expectedClass === 'solid') currentSolidPoints++;
        else if (expectedClass === 'energy') currentEnergyPoints++;
    ...
    while (currentBasicPoints > pointsToRender) {
        await render.removePoint(pointsContainer, POINT_CLASSES['points']);
        currentBasicPoints--;
    }
    while (currentSolidPoints > solidPointsToRender) {
        await render.removePoint(pointsContainer, POINT_CLASSES['solid_points']);
        currentSolidPoints--;
    }
    while (currentEnergyPoints > energyPointsToRender) {
        await render.removePoint(pointsContainer, POINT_CLASSES['energy_points']);
        currentEnergyPoints--;
    }
